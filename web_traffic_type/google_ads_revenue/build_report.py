...
rows=list(csv.DictReader((B.parent/'GulongPH Web _Untitled Page_Table (2).csv').open(encoding='utf-8-sig')))
...
for r in rows:
    ...
    out.append({**r,'decoded_order_id':oid,'match_status':'Matched' if o else 'Unmatched',**{k:v for k,v in o.items() if k!='order_id'},'fulfilled_net_revenue':o.get('net_sales_amount') if good else '0.00'})
assert len(set(x['decoded_order_id'] for x in out))==len(out)
assert all(x.get('net_sales_amount') is not None for x in out)
def write(name,rs):
...
def summarize(key):
    ...
    for k,rs in groups.items():
        result.append({key:k,'unique_orders':len(rs),'purchase_events':sum(int(r['Ecommerce purchases']) for r in rs),'net_sales_all_orders':str(sum(Decimal(r['net_sales_amount']) for r in rs)),'fulfilled_orders':sum(r['is_fulfillment_success_order']=='true' for r in rs),'fulfilled_net_revenue':str(sum(Decimal(r['fulfilled_net_revenue']) for r in rs))})
