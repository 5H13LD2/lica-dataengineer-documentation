#standardSQL
-- Medallion-aligned build for inquiry -> booking reporting by CS and Taira.
-- Scope:
--   - Taira (Chatbot/JCo)
--   - Rem Reyes
--   - Aira L. Garcia
...
WITH inquiry_cohorts AS (
  SELECT
    ...
    CASE
      ...
      WHEN LOWER(TRIM(COALESCE(agent_reporting_name, assigned_agent_name, ''))) IN ('rem reyes')
        THEN 'Rem Reyes'
      WHEN LOWER(TRIM(COALESCE(agent_reporting_name, assigned_agent_name, ''))) IN ('aira l. garcia', 'aira')
        THEN 'Aira L. Garcia'
      WHEN LOWER(TRIM(COALESCE(agent_reporting_name, assigned_agent_name, ''))) IN ('rolyn ang')
        THEN 'Rolyn Ang'
      WHEN LOWER(TRIM(COALESCE(agent_reporting_name, assigned_agent_name, ''))) IN ('sarah gulongph', 'sarah mae manansala', 'sarah')
        THEN 'Sarah'
