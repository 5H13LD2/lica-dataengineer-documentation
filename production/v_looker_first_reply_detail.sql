-- =============================================================================
-- MATERIALIZATION: t_first_reply_detail  (+ rename-swap refresh + passthrough view)
--
-- WHY: v_looker_first_reply_detail re-scans manychat_data.messages (x3),
-- chat_analysis_data (x2), fb_inquiry_sessions (x2), and turn_trace_log on
-- EVERY Looker load. Materializing runs that heavy query ONCE per refresh;
-- dashboards then read a small partitioned table (~0 scan cost, fast).
--
-- WHAT'S NEW vs the raw view (logic UNCHANGED, only cost-safety added):
--   + Partition BY report_date, cluster BY reporting_agent_name, manychat_id
--   + Literal partition floors so require_partition_filter is satisfied and
--     pruning kicks in:
--       - turn_trace_log:   t.ts >= '2026-03-01'         [was: none -> full scan]
--       - chat_analysis:    evaluation_datetime >= '2026-03-01'  [was: none]
--       - chatbot_assign.:  assignment_date >= '2026-03-01' (bounds the cohort
--         so the two floors above don't change results — see note below)
--
-- VERIFY BEFORE RUN:
--   1. Partition columns:
--        manychat_data.messages        -> assumed `datetime`
--        chat_analysis.chat_analysis_data -> assumed `evaluation_datetime`
--        gulong_chatbot_live.turn_trace_log -> assumed `ts`
--      If any table's partition column differs, adjust that floor literal.
--   2. The `assignment_date >= '2026-03-01'` floor DROPS pre-March report_dates.
--      Those rows already have NULL customer-message / reply data because your
--      existing messages floor starts at 2026-03-01, so they only surface as
--      "No CS Reply" noise. If you truly need pre-March history, comment out the
--      marked line AND remove the turn_trace_log/chat_analysis floors (accepting
--      full scans on those two).
--   3. To change the history window, find/replace '2026-03-01'.
--
-- HOW TO DEPLOY (run in BigQuery Console — MCP is read-only):
--   - Run STEP 1 once now to bootstrap the table (DROP IF EXISTS handles a
--     missing prod on first run).
--   - Run STEP 2 once to create the passthrough view.
--   - Schedule STEP 1 (daily, off-hours) via a Scheduled Query for refresh.
-- =============================================================================


-- =============================================================================
-- STEP 1 — REFRESH (rename-swap). Bootstraps on first run; schedule for refresh.
-- Build into _staging, then atomically-ish swap into the prod table name.
-- =============================================================================

CREATE OR REPLACE TABLE
  `gulong-chatbot-459723.gulong_reporting.t_first_reply_detail_staging`
PARTITION BY report_date
CLUSTER BY reporting_agent_name, manychat_id
AS
WITH chatbot_assignments AS (
  ...
  FROM (
    ...
    WHERE a.business_unit = 'gulong'
      ...
      AND a.assignment_date >= DATE '2026-03-01'   -- [COST] cohort floor (see VERIFY #2; comment out for full history)
  )
  WHERE rn = 1
),
...
chat_analysis_chatbot_intents AS (
  ...
  JOIN (
    ...
    WHERE platform = 'manychat'
      AND evaluation_datetime >= '2026-03-01'   -- [COST] partition floor (VERIFY #1)
  ) ca
...
v7_runtime_chatbot_intents AS (
  ...
  WHERE t.ts >= DATETIME '2026-03-01 00:00:00'   -- [COST] partition floor (VERIFY #1)
    AND (
      REGEXP_CONTAINS(
...
contact_from_analysis AS (
  ...
  WHERE a.evaluation_datetime >= '2026-03-01'   -- [COST] partition floor (VERIFY #1)
    AND NULLIF(TRIM(a.extracted_data.contact_number), '') IS NOT NULL
...
cohort AS (
  SELECT
    ...
    c.corrected_source,
    c.moderate_tagged_at,
    -- SLA CLOCK: CS hours 9am-6pm, lunch 12pm-1pm, Mon-Sun (simultaneous lunch)
    CASE
...
agent_messages AS (
  ...
  LEFT JOIN messages_filtered m
   ...
   -- reply window: RAW moderate_tagged_at (para mabilang ang after-hours/lunch replies)
   AND (
...
SELECT
  ...
  b.agent_key,
  b.total_inquiries,
  b.total_inquiries AS chatbot_jco_total_inquiries,
  b.total_moderate_intents,
  b.total_moderate_intents AS chatbot_jco_total_moderate_intents,
  b.total_moderate_sessions,
  b.total_moderate_sessions AS chatbot_jco_total_moderate_sessions,
  b.total_bookings,
  b.total_bookings AS chatbot_jco_total_bookings,
  ...
  CASE
    ...
    ELSE 'Has CS Reply'
  END AS reply_status,
  -- RAW clocks (reference lang, huwag gamitin sa SLA)
  ...
  -- SLA clock: effective start, BAWAS lunch overlap, floored 0 (ITO ang gamitin sa reporting)
  GREATEST(
  ...
  -- same-day-only variant (NULL kung sinagot kinabukasan) — para sa "today's SLA" scorecard
  CASE
...
-- --- swap staging -> prod (rename-swap) ---
DROP TABLE IF EXISTS `gulong-chatbot-459723.gulong_reporting.t_first_reply_detail`;
ALTER TABLE `gulong-chatbot-459723.gulong_reporting.t_first_reply_detail_staging`
  RENAME TO t_first_reply_detail;


-- =============================================================================
-- STEP 2 — PASSTHROUGH VIEW (run ONCE, after the first STEP 1 refresh).
-- Looker keeps pointing at v_looker_first_reply_detail; only the source changes.
-- =============================================================================
...
-- =============================================================================
-- ALTERNATIVE (zero-downtime, simpler): skip the DROP + RENAME in STEP 1 and
-- instead build straight into prod. CREATE OR REPLACE TABLE is transactional in
-- BigQuery — on query failure the existing table is untouched, and readers never
-- see a partial table. Trade-off: you lose the staging validation checkpoint.
--
--   CREATE OR REPLACE TABLE `...gulong_reporting.t_first_reply_detail`
--   PARTITION BY report_date
--   CLUSTER BY reporting_agent_name, manychat_id
--   AS <same SELECT>;
-- =============================================================================
