...
agent_messages AS (
  ...
  LEFT JOIN messages_filtered m
   ...
   AND (
     c.effective_moderate_tagged_at IS NULL
     OR m.agent_reply_at >= c.effective_moderate_tagged_at
   )
   AND (
...
SELECT
  ...
  DATETIME_DIFF(fr.first_cs_reply_at, fr.effective_moderate_tagged_at, MINUTE) AS minutes_from_tagged_to_first_reply,
  1 AS moderate_count,
